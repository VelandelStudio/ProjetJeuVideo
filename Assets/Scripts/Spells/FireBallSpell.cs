...
public class FireBallSpell : Spell {
    ...
    public override void LaunchSpell() {
		...
		rayOrigin = cameraPlayer.ViewportToWorldPoint(new Vector3 (0.5f,0.5f,0));
		hasFoundHitPoint = Physics.Raycast(rayOrigin, cameraPlayer.transform.forward, out hit, Mathf.Infinity, Physics.DefaultRaycastLayers, QueryTriggerInteraction.Ignore);
		if(hasFoundHitPoint) 
			target = hit.point;
		else 
