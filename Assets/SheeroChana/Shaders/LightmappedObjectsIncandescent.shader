...
// Light-mapped objects with incandescence map for GPD-Q9 V1.1
// (c) 2016 Franck Sauer
//
// RGB_nX -> RGB base color, normal X
// MGC_nY -> Metalness, Glossiness, Cavity, normal Y
// iRGB -> RGB incandescence

//updates
//1.1 added _LightmapHDRFactor to compensate for lack of HDR lightmaps on android
//1.2 added IncandescenceFactor to enable incandescence animation

Shader "Custom/LightmappedObjectsIncandescent"
{
	Properties
	{
		[NoScaleOffset] _RGB_nX ("RGB_nX", 2D) = "" {}
		[NoScaleOffset] _MGC_nY ("MGC_nY", 2D) = "" {}
		[NoScaleOffset] _iRGB ("iRGB", 2D) = "" {}
		_iFactor ("Incandescence Factor", Range (0.0,2.0)) = 1.0
	...
	SubShader
	{
		...
		Pass
		{
			...
			sampler2D _RGB_nX, _MGC_nY, _iRGB;
			...
			half4 frag(v2f i) : SV_Target
			{
				...
				half3 incand = tex2D(_iRGB,i.UV).rgb * _iFactor;
				...
				//final compo
				half3 result = (diffuse + specular) * lightmap + ambSpec + incand;
