...
// Light-mapped objects with transparency(opacity) for GPD-Q9 V1.1
// (c) 2016 Franck Sauer
//
// RGB_nX -> RGB base color, normal X
// MGO_nY -> Metalness, Glossiness, Opacity, normal Y.

//updates
//1.1 added _LightmapHDRFactor to compensate for lack of HDR lightmaps on android

Shader "Custom/LightmappedObjectsTransparent"
{
	Properties
	{
		[NoScaleOffset] _RGB_nX ("RGB_nX", 2D) = "" {}
		[NoScaleOffset] _MGO_nY ("MGO_nY", 2D) = "" {}
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag(v2f i) : SV_Target
			{
				...
				half4 MGC_nY = tex2D(_MGO_nY,i.UV).rgba;
				...
				half opacity = MGC_nY.b;
				half3 lightmap = DecodeLightmap(UNITY_SAMPLE_TEX2D(unity_Lightmap, i.UV.zw)) * _LightmapHDRFactor;
				
				//diffuse term
				half3 diffuse = (abs(NdotL*_Directionnality)+(1-_Directionnality)) * baseColor * (1-metalness); //soft double direction for non-directional lightmapped objects
